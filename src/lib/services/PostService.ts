...
import type { Category, Post } from "#types/payload";
...
export const getBySlug = cache(async ({ slug }: PostQueryArgs) => {
...
export const getPostLayoutData = cache(async ({ slug }: { slug: string }) => {
  const payload = await getPayloadClient();
...
export async function getAllSlugs(): Promise<PostQueryParams[]> {
  const payload = await getPayloadClient();
...
export const queryArchivePosts = cache(async (args: ArchivePostsArgs) => {
