...
export class MainCanvas extends Canvas {
  ...
  private get hoverColor(): string {
    return this.colorScheme === "light"
      ? getCssVariable("--color-aged-sandstone")
      : getCssVariable("--color-ironwood");
  }
  ...
  private drawHover(timeStamp: DOMHighResTimeStamp) {
    ...
    this.squares.forEach((square) => {
      ...
      const sizeRatio = SquareConfig.squareSize / SQUARE_SIZE_SMALL;
      const dx = (x - square.xPos - SquareConfig.squareSize / 2) / sizeRatio;
      const dy = (y - square.yPos - SquareConfig.squareSize / 2) / sizeRatio;
   ...
   */
  public setSquares() {
    const numX = Math.ceil(this.canvas.width / SquareConfig.squareSize);
    const numY = Math.ceil(this.canvas.height / SquareConfig.squareSize);

    this.squares = Array.from({ length: numX * numY }, (_, i) => {
      const x = i % numX;
      const y = Math.floor(i / numX);
      const xPos = x * SquareConfig.squareSize;
      const yPos = y * SquareConfig.squareSize;
  ...
  private drawLine(i: number, opacity: number) {
    ...
    const lineHeight = SquareConfig.squareSize;
    const color =
      this.colorScheme === "light"
        ? getCssVariable("--color-warm-porcelain")
        : getCssVariable("--color-shadow-earth");

    this.ctx.fillStyle = alpha(color, opacity);
    this.ctx.fillRect(0, i * SquareConfig.squareSize, width, lineHeight);
  }
